...
def compute_risk(posts: list[models.Post]) -> float:
    if not posts:
        return 0.0
    toxicity = np.mean([p.toxicity or 0.0 for p in posts])
    anti_ratio = np.mean([1.0 if (p.stance == "anti") else 0.0 for p in posts])
...
def burst_score(posts: list[models.Post]) -> float:
    ...
    intervals = np.diff(timestamps) if len(timestamps) > 1 else np.array([3600.0])
    mu = float(np.mean(intervals))
    sigma = float(np.std(intervals) + 1e-6)
...
def coordination_score(posts: list[models.Post]) -> float:
    ...
    if emb.shape[0] < 3:
        return 0.0
    sim = np.clip(np.matmul(emb, emb.T), 0, 1)
    n = sim.shape[0]
    # fraction of pairs with similarity > 0.85 within 15 minutes
    times = np.array([p.created_at.timestamp() for p in posts])
    coord_pairs = 0
    total_pairs = max(1, n * (n - 1) // 2)
    for i in range(n):
        for j in range(i + 1, n):
            if sim[i, j] > 0.85 and abs(times[i] - times[j]) <= 900:
                coord_pairs += 1
